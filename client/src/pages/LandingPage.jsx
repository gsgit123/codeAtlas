...
export default function LandingPage() {
    ...
    const pollStatus = (projectId) => {
        const iv = setInterval(async () => {
            try {
                ...
                clearInterval(iv);
                setStatus('error');
                setStatusMsg('server error ',error)
    ...
    const handleUpload=async()=>{
        ...
        setStatus('uploading');
        setStatusMsg('Uploading repository...');
        try {
            ...
            setStatus('error'); 
            setStatusMsg('Upload failed. Are both servers running?',error)
        }
    }
    const statusColor={
    ...
    <div className="min-h-screen bg-slate-950 text-white flex flex-col items-center justify-center px-6 py-20">
      {/* Nav */}
      <nav className="fixed top-0 left-0 right-0 flex justify-between items-center px-10 py-5 border-b border-slate-800 bg-slate-950/80 backdrop-blur-md z-50">
        <span className="text-xl font-black tracking-tight gradient-text">⬡ CodeAtlas</span>
        <button onClick={() => navigate('/dashboard')}
          className="text-sm font-medium text-slate-400 border border-slate-700 px-4 py-2 rounded-lg hover:border-indigo-500 hover:text-white transition-all">
          My Projects →
        </button>
      </nav>
      {/* Hero */}
      <div className="text-center max-w-2xl mb-14 fade-in">
        <div className="inline-flex items-center gap-2 bg-indigo-500/10 border border-indigo-500/30 rounded-full px-4 py-1.5 text-xs text-indigo-300 mb-7 font-medium">
      ...
      <div className="w-full max-w-lg fade-in">
        <div
          ...
          className={`border-2 border-dashed rounded-2xl p-16 text-center cursor-pointer transition-all duration-300
            ${dragOver ? 'border-indigo-500 bg-indigo-500/10 shadow-[0_0_40px_rgba(99,102,241,0.15)]'
                       : 'border-slate-700 bg-slate-900 hover:border-indigo-500 hover:bg-indigo-500/5'}`}
        >
          ...
          {file ? (
            <div>
              ...
              <p className="text-slate-500 text-sm mb-4">{(file.size / 1024).toFixed(1)} KB</p>
              <button onClick={e => { e.stopPropagation(); setFile(null) }}
                className="text-xs border border-slate-700 text-slate-400 px-3 py-1.5 rounded-md hover:border-slate-500">
                Remove
              </button>
            </div>
          ) : (
        ...
        {file && status === 'idle' && (
          <button onClick={handleUpload}
            className="w-full mt-4 py-4 bg-indigo-600 hover:bg-indigo-500 active:scale-[0.99] text-white font-semibold rounded-xl transition-all text-base">
            Analyse Repository →
          </button>
        )}
        {/* Status Bar */}
        {status !== 'idle' && (
          <div className="mt-5 p-5 bg-slate-900 border border-slate-800 rounded-xl">
            <div className="flex items-center gap-3">
              {(status === 'uploading' || status === 'processing') && (
                <div className={`w-2 h-2 rounded-full pulse-dot ${status === 'uploading' ? 'bg-indigo-400' : 'bg-amber-400'}`} />
              )}
            ...
            {status === 'processing' && (
              <p className="text-slate-600 text-xs mt-3 leading-relaxed">
                This may take 20–60s depending on codebase size. The AI is parsing, graphing, and embedding your code.
              </p>
            )}
          </div>
        )}
      ...
      <div className="flex gap-3 flex-wrap justify-center mt-14 fade-in">
        {['⚡ AST Parsing', '🕸 Dependency Graph', '🧠 Vector Embeddings', '🔍 Hybrid Search', '🤖 AI Answers'].map(f => (
          <span key={f} className="bg-slate-900 border border-slate-800 rounded-lg px-4 py-2 text-xs text-slate-400">
            {f}
          </span>
        ))}
      </div>
